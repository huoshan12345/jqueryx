...
beforeAll(() => {
  ...
  const packageDirectory = join(consumerDirectory, 'node_modules', '@huoshan12345', 'jqueryx');
  ...
  const exampleFiles = examples.map((example, index) => {
    ...
    writeFileSync(join(consumerDirectory, file), `import '@huoshan12345/jqueryx';\n${example[1]}\nexport {};\n`);
    return file;
  });
...
test('the published declaration graph uses explicit relative ESM paths', () => {
  const entry = readFileSync(join(consumerDirectory, 'node_modules/@huoshan12345/jqueryx/dist/index.d.ts'), 'utf8');
